...
const NutritionManagement = ({ navigation }) => {
  ...
  const fetchInsights = async () => {
    try {
      const response = await axios.get("http://localhost:5000/nutritions"); // Replace with your machine's IP address
  ...
  const handleDeleteInsight = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/nutritions/${id}`);
