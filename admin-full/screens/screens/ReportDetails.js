...
const ReportDetails = ({ route, navigation }) => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/reports/${reportId}`)
      .then(response => {
  ...
  const handleMarkInvalid = () => {
    axios.put(`http://localhost:5000/reports/${reportId}`, { status: 'invalid' })
      .then(() => {
  ...
  const handleDeletePost = () => {
    axios.put(`http://localhost:5000/reports/${reportId}`, { status: 'resolved' })
      .then(() => {
