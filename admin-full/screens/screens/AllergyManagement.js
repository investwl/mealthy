...
const AllergyManagement = ({ navigation }) => {
  ...
  const fetchAllergies = async () => {
    try {
      const response = await axios.get("http://localhost:5000/allergies"); // Fetch allergy data from the server
              ...
              <View style={styles.allergyCard}>
                <View style={styles.allergyDetails}>
                  ...
                  <Text style={styles.allergyName}>
                    {Array.isArray(item.name) ? item.name.join(', ') : 'No allergens listed'}
                  </Text>
                </View>
